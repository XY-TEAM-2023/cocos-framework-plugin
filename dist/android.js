...
exports.generateMultiEnvApks = exports.getEnabledEnvironments = exports.getEffectiveConfig = exports.saveAndroidConfig = exports.loadAndroidConfig = void 0;
...
/** 加载 Android 打包配置 */
function loadAndroidConfig(projectRoot) {
...
exports.loadAndroidConfig = loadAndroidConfig;
/** 保存 Android 打包配置 */
function saveAndroidConfig(projectRoot, config) {
    const configPath = getConfigPath(projectRoot);
    fs.writeFileSync(configPath, JSON.stringify(config, null, 2), 'utf-8');
}
exports.saveAndroidConfig = saveAndroidConfig;
...
function getEnabledEnvironments(config) {
    ...
    if (config.environments.dev)
        envs.push('dev');
    if (config.environments.beta)
        envs.push('beta');
    if (config.environments.prod)
        envs.push('prod');
    return envs;
}
exports.getEnabledEnvironments = getEnabledEnvironments;
// ==================== env.json 写入 ====================
/**
 ...
 * 超时 10 分钟
 */
function runGradleAssemble(projDir, onLog) {
    return new Promise((resolve, reject) => {
        ...
        const child = (0, child_process_1.spawn)(gradlew, ['assembleRelease', '--no-daemon'], {
            cwd: projDir,
 ...
 */
function findBuiltApk(buildDest) {
    // 候选路径 1: gradle 直接输出
 ...
 * 4. 清理 env.json
 */
async function generateMultiEnvApks(options) {
...
exports.generateMultiEnvApks = generateMultiEnvApks;
