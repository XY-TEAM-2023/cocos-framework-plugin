...
function ready() {
    treeContainer = this.$['tree-container'];
    progressContainer = this.$['progress-container'];
    progressBar = this.$['progress-bar'];
    progressText = this.$['progress-text'];
    statusText = this.$['status-text'];
    btnUpload = this.$['btn-upload'];
    btnCancel = this.$['btn-cancel'];
    btnSelectAll = this.$['btn-select-all'];
    btnDeselectAll = this.$['btn-deselect-all'];
...
exports.methods = {
    ...
    setUploading(isUploading) {
        ...
        if (btnUpload)
            btnUpload.style.display = uploading ? 'none' : 'block';
        if (btnCancel)
            btnCancel.style.display = uploading ? 'block' : 'none';
        if (btnSelectAll)
            btnSelectAll.style.display = uploading ? 'none' : 'inline-block';
        if (btnDeselectAll)
            btnDeselectAll.style.display = uploading ? 'none' : 'inline-block';
        if (uploading) {
            // 清空之前的日志
            const logEl = progressContainer === null || progressContainer === void 0 ? void 0 : progressContainer.querySelector('#upload-log');
            if (logEl)
                logEl.innerHTML = '';
            if (progressBar) {
