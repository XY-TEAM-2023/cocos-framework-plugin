...
function renderEnvRow(key, label, defaultMethod, defaultChecked) {
    const methodOptions = [
    ...
    const optionsHtml = methodOptions.map(o => `<option value="${o.value}"${o.value === defaultMethod ? ' selected' : ''}>${o.label}</option>`).join('');
    return `
...
function updateMethodHint(panel, envKey) {
    const methodEl = panel.$[`env-${envKey}-method`];
    const hintEl = panel.$[`env-${envKey}-hint`];
    if (methodEl && hintEl) {
...
function ready() {
    ...
    const btnBuild = this.$['btn-build'];
    const btnCopy = this.$['btn-copy'];
    const btnClear = this.$['btn-clear'];
    // 环境勾选 + 导出方式变化
    for (const envKey of ['dev', 'beta', 'prod']) {
        const checkbox = this.$[`env-${envKey}`];
        const methodEl = this.$[`env-${envKey}-method`];
    ...
    btnBuild.addEventListener('click', () => {
        ...
        Editor.Message.send('framework-plugin', 'start-ios-build', JSON.stringify({
            ...
            exportMethods: envs.reduce((acc, e) => { acc[e.env] = e.exportMethod; return acc; }, {}),
        }));
    });
    // 复制按钮
...
exports.ready = ready;
function appendLocalLog(message, type) {
...
exports.methods = {
    ...
    loadConfig(dataStr) {
        try {
            ...
            // 签名状态
            const statusEl = this.$['signing-status'];
            const btnBuild = this.$['btn-build'];
