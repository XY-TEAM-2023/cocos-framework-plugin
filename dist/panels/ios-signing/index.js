...
exports.ready = ready;
function setStatusText(panel, text, color) {
    const el = panel.$['status-text'];
    if (el) {
        el.textContent = text;
...
exports.methods = {
     ...
     */
    setMobileProvisionResult(dataStr) {
        try {
            const data = JSON.parse(dataStr);
            const envKey = data.envKey; // 'dev' | 'beta' | 'prod'
            if (!envKey || !envProfiles[envKey])
                return;
            envProfiles[envKey] = {
            ...
            // 更新 UI
            const mpFileEl = this.$[`env-${envKey}-mp-file`];
            if (mpFileEl) {
                mpFileEl.textContent = data.fileName;
                mpFileEl.style.color = '#4ec9b0';
            }
            const mpInfoEl = this.$[`env-${envKey}-mp-info`];
            if (mpInfoEl)
                mpInfoEl.style.display = 'block';
            const mpNameEl = this.$[`env-${envKey}-mp-name`];
            if (mpNameEl)
                mpNameEl.textContent = data.name || '';
            const mpBundleIdEl = this.$[`env-${envKey}-mp-bundleid`];
            if (mpBundleIdEl)
                mpBundleIdEl.textContent = data.bundleId || '';
            const mpExpiredEl = this.$[`env-${envKey}-mp-expired`];
            if (mpExpiredEl)
                mpExpiredEl.style.display = data.expired ? 'inline' : 'none';
            // 自动填充 Team ID（仅首次）
            if (data.teamId) {
                const teamIdInput = this.$['team-id'];
     ...
     * 接收 P12 选择结果
     */
    setP12Result(dataStr) {
        try {
            const data = JSON.parse(dataStr);
            p12File = data.fileName || '';
            const el = this.$['p12-file'];
            if (el) {
                el.textContent = p12File;
    ...
    loadConfig(configStr) {
        ...
        try {
            ...
            // 共享配置
            if ((_a = config.shared) === null || _a === void 0 ? void 0 : _a.p12File) {
            ...
            if ((_b = config.shared) === null || _b === void 0 ? void 0 : _b.p12Password) {
                const el = this.$['p12-password'];
                if (el)
                    el.value = config.shared.p12Password;
            }
            if ((_c = config.shared) === null || _c === void 0 ? void 0 : _c.teamId) {
                const el = this.$['team-id'];
                if (el)
                    el.value = config.shared.teamId;
            }
            // 各环境描述文件
            for (const env of ENV_LIST) {
                const envCfg = (_d = config.environments) === null || _d === void 0 ? void 0 : _d[env.key];
                if (!envCfg)
                    continue;
                if (envCfg.mobileprovisionFile) {
                    ...
                    const mpFileEl = this.$[`env-${env.key}-mp-file`];
                    if (mpFileEl) {
                        mpFileEl.textContent = envCfg.mobileprovisionFile;
                        mpFileEl.style.color = '#4ec9b0';
                    }
                    if (envCfg.profileName) {
                        const mpInfoEl = this.$[`env-${env.key}-mp-info`];
                        if (mpInfoEl)
                            mpInfoEl.style.display = 'block';
                        const mpNameEl = this.$[`env-${env.key}-mp-name`];
                        if (mpNameEl)
                            mpNameEl.textContent = envCfg.profileName;
                        const mpBundleIdEl = this.$[`env-${env.key}-mp-bundleid`];
                        if (mpBundleIdEl)
                            mpBundleIdEl.textContent = envCfg.bundleId || '';
                    }
                }
            }
        }
