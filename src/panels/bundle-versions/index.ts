...
interface BundleTreeNode {
    platform: string;
    bundleName: string;
    versions: { dev?: string; beta?: string; prod?: string };
}

let treeData: BundleTreeNode[] = [];
...
function updateActionBar() {
    const actionBar = panelRef?.$['action-bar'] as HTMLElement;
    if (!actionBar) return;
    actionBar.style.display = activePlatform ? 'flex' : 'none';
}

function setApplyButtonsDisabled(disabled: boolean) {
    const actionBar = panelRef?.$['action-bar'] as HTMLElement;
    if (!actionBar) return;
...
function renderTree() {
    const container = panelRef?.$['tree-container'] as HTMLElement;
    if (!container) return;
    ...
    container.querySelectorAll('.bv-version-cell').forEach(cell => {
        cell.addEventListener('click', () => {
            const bundleName = (cell as HTMLElement).dataset.bundle!;
            const env = (cell as HTMLElement).dataset.env! as any;
            const current = (cell as HTMLElement).dataset.current!;
            openVersionSelector(bundleName, env);
        });
    });
}

function renderVersionCell(node: BundleTreeNode, env: 'dev' | 'beta' | 'prod'): string {
...
function openVersionSelector(bundleName: string, env: 'dev' | 'beta' | 'prod') {
    ...
    // 显示弹层
    const overlay = panelRef?.$['version-selector'] as HTMLElement;
    if (overlay) overlay.style.display = 'block';

    const titleEl = panelRef?.$['vs-title'] as HTMLElement;
    if (titleEl) titleEl.textContent = `设置 [${bundleName}] 的 ${env.toUpperCase()} 环境版本`;

    // 清空列表显示加载中
    const list = panelRef?.$['vs-version-list'] as HTMLElement;
    if (list) list.innerHTML = '<div style="text-align: center; color: #888; padding: 20px; font-size: 12px;">加载版本列表中...</div>';
...
function renderVersionList() {
    ...
    list.querySelectorAll('.vs-ver-item').forEach(el => {
        el.addEventListener('click', () => {
            ...
            const ver = (el as HTMLElement).dataset.ver!;
            // 如果点击当前版本，取消选择
    ...
    if (btnLoadMore) {
        btnLoadMore.addEventListener('click', () => {
            if (!selectorState || selectorState.loadingMore) return;
...
function updateConfirmButton() {
    const btn = panelRef?.$['vs-confirm'] as HTMLButtonElement;
    if (!btn || !selectorState) return;
...
export function ready(this: any) {
    ...
    // 取消按钮
    const btnCancel = this.$['vs-cancel'] as HTMLElement;
    btnCancel.addEventListener('click', closeSelector);

    // 确定按钮
    const btnConfirm = this.$['vs-confirm'] as HTMLButtonElement;
    btnConfirm.addEventListener('click', async () => {
        if (!selectorState || !selectorState.selectedVersion) return;
        const { platform, bundleName, env, selectedVersion } = selectorState;

        const result = await Editor.Dialog.info(
            `是否确定将 ${platform} 平台的 ${bundleName}\n的 ${env} 版本切换为 ${selectedVersion}？`,
            { buttons: ['确定', '取消'], default: 0, cancel: 1 }
        );
    ...
    // 一键应用最新版本按钮
    const actionBar = this.$['action-bar'] as HTMLElement;
    actionBar.querySelectorAll('.apply-btn').forEach((btn: Element) => {
        btn.addEventListener('click', async () => {
            ...
            const result = await Editor.Dialog.info(
                `确定要将 ${activePlatform} 平台下所有 Bundle 的最新版本应用到 ${envLabel} 环境吗？`,
                { buttons: ['确定', '取消'], default: 0, cancel: 1 }
            );
...
export const methods = {
    setBundlePlatforms(jsonStr: string) {
        console.log('[Bundle版本管理] 收到平台列表数据');
        try {
            const platforms = JSON.parse(jsonStr) as string[];
            if (platforms.length === 0) {
                console.warn('[Bundle版本管理] 平台列表为空');
                const tabsCont = panelRef?.$['platform-tabs'] as HTMLElement;
                if (tabsCont) tabsCont.innerHTML = '<div style="padding: 8px 16px; color: #888; font-size: 13px;">没有找到任何平台</div>';
                return;
            }
            ...
            if (tabsCont) {
                ...
                tabsCont.querySelectorAll('.tab-btn').forEach(btn => {
                    btn.addEventListener('click', () => {
                        const newPlatform = (btn as HTMLElement).dataset.platform!;
                        ...
                        // 显示加载中并请求数据
                        const container = panelRef?.$['tree-container'] as HTMLElement;
                        if (container) container.innerHTML = '<div style="text-align: center; color: #888; padding: 40px;">加载中...</div>';
    ...
    setBundleTree(jsonStr: string) {
        console.log('[Bundle版本管理] 收到 Bundle 树数据');
        try {
            treeData = JSON.parse(jsonStr) as BundleTreeNode[];
    ...
    setBundleVersionList(jsonStr: string) {
        if (!selectorState) return;
        try {
            const data = JSON.parse(jsonStr) as { versions: string[], nextContinuationToken?: string };
            if (selectorState.loadingMore) {
                // 追加
            ...
            if (!selectorState.loadingMore) selectorState.availableVersions = [];
        } finally {
    ...
    switchBundleVersionResult(success: boolean, msg: string) {
        if (success) {
            // 刷新当前平台的树
            if (activePlatform) {
                Editor.Message.send('framework-plugin', 'load-bundle-tree-by-platform', activePlatform);
    ...
    applyLatestResult(jsonStr: string) {
        setApplyButtonsDisabled(false);
        try {
            const data = JSON.parse(jsonStr) as { success: boolean; message: string };
            if (data.success) {
