...
export function ready(this: any) {
    ...
    const btnLoadMore = panelRef?.$['btn-load-more'] as HTMLButtonElement;
    if (btnLoadMore) {
        btnLoadMore.onclick = () => {
...
function renderDeployments(deployments: any[], isAppend: boolean) {
    const list = panelRef?.$['deployment-list'] as HTMLElement;
    const table = panelRef?.$['deployment-table'] as HTMLElement;
    const loading = panelRef?.$['loading'] as HTMLElement;
    const emptyHint = panelRef?.$['empty-hint'] as HTMLElement;
    const loadMoreContainer = panelRef?.$['load-more-container'] as HTMLElement;
    const noMoreHint = panelRef?.$['no-more-hint'] as HTMLElement;

    if (!list || !table || !loading) return;
    ...
    if (currentDeployments.length === 0) {
        ...
        if (noMoreHint) noMoreHint.style.display = 'none';
        return;
    }
    ...
    if (noMoreHint) noMoreHint.style.display = (!hasMore && currentDeployments.length > 0) ? 'block' : 'none';
    ...
    const rowsHtml = deployments.map((d: any) => {
...
export const methods = {
    setVersionsData(dataStr: string) {
        try {
            ...
            const statusBar = panelRef?.$['status-bar'] as HTMLElement;
            if (statusBar) statusBar.textContent = '共 ' + currentDeployments.length + ' 条记录';
        } catch (e) {
