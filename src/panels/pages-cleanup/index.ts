...
function updateSelectedCount() {
    ...
    const countEl = panelRef?.$['selected-count'] as HTMLElement;
    const btnCleanup = panelRef?.$['btn-cleanup'] as HTMLButtonElement;
    if (!list || !countEl || !btnCleanup) return;
...
function getSelectedIds(): string[] {
    ...
    if (!list) return [];
    const checked = list.querySelectorAll('input[type="checkbox"]:checked');
    return Array.from(checked).map(el => (el as HTMLInputElement).value);
}
...
function renderDeployments(deployments: any[]) {
    ...
    list.innerHTML = deployments.map((d: any) => {
        ...
        const icon = d.is_current ? '🟢' : d.latest_stage?.status === 'success' ? '✅' : '❌';
...
export function ready(this: any) {
    ...
    const btnCleanup = this.$['btn-cleanup'] as HTMLElement;
    btnCleanup.addEventListener('click', () => {
        ...
        Editor.Dialog.warn(
            ...
            { buttons: ['确认清理', '取消'], default: 0, cancel: 1, title: '⚠️ 清理确认' }
        ).then((result: any) => {
...
export const methods = {
    ...
    /** 更新清理进度 */
    setCleanupProgress(dataStr: string) {
        try {
    ...
    /** 清理完成 */
    setCleanupComplete(resultStr: string) {
        try {
